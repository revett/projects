...
```bash
docker build -f build/drakov.dockerfile -t revett/projects/drakov .
```
...
docker run -it --init --rm \
  ...
  revett/projects/drakov -f /api/thruhike_content_api.apib --watch
```
...
```bash
docker build -f build/aglio.dockerfile -t revett/projects/aglio .
```
...
docker run -it --init --rm \
  ...
  revett/projects/aglio -i /api/thruhike_content_api.apib
```
